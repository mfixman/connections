...
class TestClassicalState:
    ...
    def test_start_states(self):
        ...
        assert (
            str([child.literal for child in self.state.tableau.children])
            == "[big_f(_131041), big_f(f(_131041))]"
        )
        assert str(self.state.tableau.children[0].literal) == str(
            self.state.goal.literal
        )
    ...
    def test_extensions(self):
        ...
        assert (
            str(self.state._extensions())
            == "[ex0: big_f(_131041) -> [big_f(_134445), -big_f(f(_134445))],"
...
class TestConnectionEnv:
    ...
    def test_iterative_deepening_steps(self):
        ...
        # ASSERT
        assert (
            str(action) == "ex0: big_f(_131041) -> [big_f(_134442), -big_f(f(_134442))]"
        )
        ...
        assert (
            ...
            == "[ex1: big_f(_131041) -> [-big_f(_131043), -big_f(f(_131043))],"
        ...
        assert (
            str(observation.proof_sequence)
            == "[st0: [big_f(_131041), big_f(f(_131041))]]"
        )
    ...
    def test_depth(self):
        # ARRANGE
        settings = Settings(
            iterative_deepening=True, iterative_deepening_initial_depth=3
        )
    ...
    def test_unresolved_timeout(self):
        ...
        settings = Settings(iterative_deepening=True)
        problem = (
            "tests/cnf_problems/GEO185+1.p"  # NUM531+1.p" #CSR115+20.p" #LAT381+3.p"
        )
        env = ConnectionEnv(problem, settings=settings)
        # ACT
