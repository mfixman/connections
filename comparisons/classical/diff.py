"""Compare the performance of pyCoP and leanCoP theorem provers.

This script runs both provers on a set of TPTP problems and compares their:
- Execution times
- Inference counts
- Success rates
It generates CSV files containing detailed comparison results.
"""
...
)  # Filter for TPTP problem files


def run_theorem_prover(problempath: str) -> dict:
    """Run both theorem provers on a given problem and compare their performance.

    Args:
        problempath: Path to the TPTP problem file.

    Returns:
        Dictionary containing comparison metrics between the two provers.
    """
    ...
    with open(f"{out_path}/out/{problem}_pycop.out", "w+") as f:
        with subprocess.Popen(
            ["python", pycop_path, problem], stdout=f, preexec_fn=os.setsid
        ) as process:
            try:
    ...
    with open(f"{out_path}/out/{problem}_leancop.out", "w+") as f:
        with subprocess.Popen(
            [leancop_path, problempath], stdout=f, preexec_fn=os.setsid
        ) as process:
            try:
                output, errors = process.communicate(timeout=1)
                if output is not None:
    ...
    with (
        open(f"{out_path}/out/{problem}_pycop.out") as file_1,
        open(f"{out_path}/out/{problem}_leancop.out") as file_2,
    ):
    ...
    with (
        open(f"{out_path}/log/{problem}_pycop.log", "r") as file_1,
        open(f"{out_path}/log/{problem}_leancop.log", "r") as file_2,
    ):
...
if __name__ == "__main__":
    ...
    df2 = df[
        (df["pyCoP status"] != df["leanCoP status"]) | (df["Diff inference"].notna())
    ]
