...
def run_theorem_prover(problempath):
    ...
    with (
        open(f"{out_path}/out/{problem}_ipycop.out") as file_1,
        open(f"{out_path}/out/{problem}_ileancop.out") as file_2,
    ):
    ...
    with (
        open(f"{out_path}/log/{problem}_ipycop.log", "r") as file_1,
        open(f"{out_path}/log/{problem}_ileancop.log", "r") as file_2,
    ):
...
if __name__ == "__main__":
    ...
    # Extract diff
    df2 = df[
        (df["ipyCoP status"] != df["ileanCoP status"]) | (df["Diff inference"].notna())
    ]
