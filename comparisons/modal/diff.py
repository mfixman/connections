...
logics = itertools.product(
    ["S4", "S5", "D", "T"], ["constant", "cumulative", "varying"]
)
fof_filter = (
    lambda filename: "+" in filename
    and ".p" in filename
    and filename.count(".") == 1
    and "MML" not in filename
)  # and 'SYM009' in filename
...
def run_theorem_prover(problempath):
    ...
    with subprocess.Popen(
        [translator_path, problempath, problem], preexec_fn=os.setsid
    ) as process:
        try:
    ...
    for logic, domain in logics:
        ...
        with open(f"{out_path}/out/{problem}_{logic}_{domain}_mpycop.out", "w+") as f:
            with subprocess.Popen(
                ["python", pycop_path, problem, logic, domain],
                stdout=f,
        ...
        with open(f"{out_path}/out/{problem}_{logic}_{domain}_mleancop.out", "w+") as f:
            with subprocess.Popen(
                [leancop_path, problempath, logic.lower(), prolog_domain],
                stdout=f,
                preexec_fn=os.setsid,
            ) as process:
                try:
        ...
        with (
            open(f"{out_path}/out/{problem}_{logic}_{domain}_mpycop.out") as file_1,
            open(f"{out_path}/out/{problem}_{logic}_{domain}_mleancop.out") as file_2,
        ):

            with open(
                f"{out_path}/log/{problem}_{logic}_{domain}_mpycop.log", "w+"
            ) as new_py:
            ...
            skip_count = 0
            with open(
                f"{out_path}/log/{problem}_{logic}_{domain}_mleancop.log", "w+"
            ) as new_lean:
        ...
        with (
            open(
                f"{out_path}/log/{problem}_{logic}_{domain}_mpycop.log", "r"
            ) as file_1,
            open(
                f"{out_path}/log/{problem}_{logic}_{domain}_mleancop.log", "r"
            ) as file_2,
        ):
            ...
            if lines_1:
                ipy_status = (
                    "Theorem" in lines_1[-1] and not "Non-Theorem" in lines_1[-1]
                )
                lines_1 = lines_1[:-1]
            if lines_2:
                ilean_status = "Theorem" in lines_2[-1]
                lines_2 = lines_2[:-2]
            info[f"mpyCoP {logic} {domain} status"] = (
                "Theorem" if ipy_status else "Timeout"
            )
            info[f"mleanCoP {logic} {domain} status"] = (
                "Theorem" if ilean_status else "Timeout"
            )
...
if __name__ == "__main__":
    ...
    for folder in os.listdir(fof_path):
        if not os.path.isdir(os.path.join(fof_path, folder)):
            continue
        problems = [
            os.path.join(fof_path, folder, filename)
            for filename in os.listdir(fof_path + folder)
            if fof_filter(filename)
        ]
    ...
    for logic, domain in logics:
        ...
        df2 = (
            df[
                (
                    df[f"mpyCoP {logic} {domain} status"]
                    != df[f"mleanCoP {logic} {domain} status"]
                )
                | df[f"Diff inference {logic} {domain}"].notna()
            ]
            .filter(regex=f"{logic} {domain}|Problem")
            .dropna()
        )
        ...
        problem_column.to_csv(
            f"problems_{logic}_{domain}.txt", index=False, header=None, sep="\t"
        )
